...
import com.example.book.databinding.EditProfileFragmentBinding
...
class EditProfileFragment : Fragment(R.layout.edit_profile_fragment) {
    ...
    private lateinit var categoryChooserViewModel: CategoryChooserViewModel
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        categoryChooserViewModel = ViewModelProvider(this).get(CategoryChooserViewModel::class.java)

        categoryChooserViewModel.user.observe(viewLifecycleOwner, observerUser)

        categoryChooserViewModel.getCurrentUser()
        setupChips()


        binding.backButton.setOnClickListener {
            this.parentFragmentManager.beginTransaction()
                .replace(R.id.containerHome, BookListingFragment())
                .commitNow()
        }
    ...
    private fun setupSaveCategoriesButton(userId: String) {
        binding.saveCategoriesButton.setOnClickListener {
            if (listOfCategories.size != 3) {
                ...
                categoryChooserViewModel.updateCategories(
                    UserCategories(userId = userId, categories = listOfCategories)
                )
                categoryChooserViewModel.getUserCategories(userId)
